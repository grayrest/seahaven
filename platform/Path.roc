...
Path := [
	...
	## Add or remove the executable bits on the file at this path.
	##
	## Mirrors `chmod +x` / `chmod -x`: execute is granted only where read is
	## already granted, so a `0o600` file becomes `0o700` rather than a
	## world-executable `0o711`.
	##
	## Returns an `Unsupported` error on Windows, where whether a file runs is
	## decided by its extension rather than a permission bit.
	##
	## ```roc
	## Path.utf8("script.sh").set_executable!(True)?
	## ```
	set_executable! : Path, Bool => Try({}, [PathErr(IOErr, Path), ..])
	set_executable! = |path, executable|
		map_file_result(Host.file_set_executable!(to_raw(path), executable), path)
	...
	## The path with every symlink and `.`/`..` resolved.
	##
	## Needs the FILESYSTEM, so it differs from a textual clean: on macOS `/tmp`
	## comes back as `/private/tmp`. The path must exist.
	##
	## ```roc
	## real = Path.canonicalize!(Path.utf8("."))?
	## ```
	canonicalize! : Path => Try(Path, [PathErr(IOErr, Path), ..])
	canonicalize! = |path|
		match Host.path_canonicalize!(to_raw(path)) {
			Ok(resolved) => Ok(from_raw(resolved))
			Err(CanonicalizeFailed) =>
				Err(PathErr(NotFound, path))
		}
