...
impl Drop for SplitWriter<'_> {
    fn drop(&mut self) {
        if self.options.elide_empty_files && self.size == 0 {
            ...
            let _ = brush_vfs::ambient::remove_file(file_name);
        }
    }
}
...
impl SplitWriter<'_> {
    ...
    fn new_writer(&mut self) -> Result<(), CsplitError> {
        let file_name = self.options.split_name.get(self.counter);
        let file = brush_vfs::ambient::create(&file_name)
    ...
    fn delete_all_splits(&self) -> io::Result<()> {
        ...
        for ith in 0..self.counter {
            ...
            if let Err(err) = brush_vfs::ambient::remove_file(file_name) {
                ret = Err(err);
            }
        }
        ret
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    if file_name == "-" {
        ...
        let file = brush_vfs::ambient::open(file_name)
