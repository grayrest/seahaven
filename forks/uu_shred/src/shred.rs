...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let random_source = match matches.get_one::<String>(options::RANDOM_SOURCE) {
        Some(filepath) => Some(RefCell::new(
            brush_vfs::ambient::open(filepath).map_err_context(|| filepath.clone())?,
        )),
        None => None,
    };
...
fn wipe_file(
    ...
    if path_str.as_encoded_bytes().ends_with(b"/") {
        ...
        if brush_vfs::ambient::metadata(path).is_err_and(|e| e.kind() == io::ErrorKind::NotADirectory) {
            return Err(USimpleError::new(
                1,
    ...
    let metadata =
        brush_vfs::ambient::metadata(path).map_err_context(|| translate!("shred-failed-to-get-metadata"))?;
    ...
    if force {
        ...
        brush_vfs::ambient::set_permissions(path, perms)
...
fn wipe_name(orig_path: &Path, verbose: bool, remove_method: RemoveMethod) -> PathBuf {
    ...
    for length in (1..=file_name_len).rev() {
        ...
        for name in FilenameIter::new(length) {
            ...
            if brush_vfs::ambient::exists(&(new_path)) {
                continue;
            }
            match brush_vfs::ambient::rename(&last_path, &new_path) {
                Ok(()) => {
                    if verbose {
                        show_error!(
                            "{}: {} {}",
...
fn do_remove(path: &Path, verbose: bool, remove_method: RemoveMethod) -> Result<(), io::Error> {
    ...
    brush_vfs::ambient::remove_file(remove_path)?;
