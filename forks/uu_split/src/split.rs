...
fn get_input_size(
    ...
    if num_bytes < read_limit {
        ...
        let metadata = brush_vfs::ambient::metadata(Path::new(input))?;
        ...
        if num_bytes <= metadata_size {
            ...
            let mut tmp_fd = brush_vfs::ambient::open(Path::new(input))?;
...
fn split(settings: &Settings) -> UResult<()> {
    let mut reader = if settings.input == "-" {
        ...
        let r = brush_vfs::ambient::open(Path::new(&settings.input)).map_err_context(
