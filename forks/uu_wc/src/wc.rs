...
impl<'a> Input<'a> {
    ...
    fn try_as_files0(&self) -> UResult<Option<Vec<Input<'static>>>> {
        match self {
            Self::Path(path) => match brush_vfs::ambient::metadata(path) {
...
fn word_count_from_input(input: &Input<'_>, settings: &Settings) -> CountResult {
    let (total, maybe_err) = match input {
        ...
        Input::Path(path) => match brush_vfs::ambient::open(path) {
        ...
        Inputs::Paths(inputs) => {
            ...
            for input in inputs {
                match input {
                    ...
                    Input::Path(path) => {
                        if let Ok(meta) = brush_vfs::ambient::metadata(path) {
                            if meta.is_file() {
                                total += meta.len();
                            } else {
...
fn files0_iter_file<'a>(path: &Path) -> UResult<impl Iterator<Item = InputIterItem<'a>>> {
    match brush_vfs::ambient::open(path) {
