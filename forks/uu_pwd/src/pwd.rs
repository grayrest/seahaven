...
fn physical_path() -> io::Result<PathBuf> {
    ...
    #[cfg(not(unix))]
    {
        brush_vfs::ambient::canonicalize(&(path))
    }
}
    ...
    #[cfg(not(windows))]
    {
        ...
        fn looks_reasonable(path: &Path) -> bool {
            ...
            #[cfg(unix)]
            {
                ...
                match (brush_vfs::ambient::metadata(path), brush_vfs::ambient::metadata(".")) {
                    (Ok(info1), Ok(info2)) => {
            ...
            #[cfg(not(unix))]
            {
                ...
                match (brush_vfs::ambient::canonicalize(path), brush_vfs::ambient::canonicalize(".")) {
