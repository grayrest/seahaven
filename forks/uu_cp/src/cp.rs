...
pub fn copy(sources: &[PathBuf], target: &Path, options: &Options) -> CopyResult<()> {
    ...
    for source in sources {
        ...
        if options.backup == BackupMode::None && seen_sources.contains(&normalized_source) {
            let file_type = if brush_vfs::ambient::symlink_metadata(&(source))?.file_type().is_dir() {
                "directory"
            } else {
                "file"
            };
            ...
            if FileInformation::from_path(&dest, true).is_ok()
                && !brush_vfs::ambient::symlink_metadata(&dest).is_ok_and(|m| m.file_type().is_symlink())
                // if both `source` and `dest` are symlinks, it should be considered as an overwrite.
                || brush_vfs::ambient::metadata(source).is_ok()
                    && brush_vfs::ambient::symlink_metadata(source)?.file_type().is_symlink()
                || matches!(options.copy_mode, CopyMode::SymLink)
            {
                ...
                if copied_destinations.contains(&dest) && options.backup != BackupMode::Numbered {
                    ...
                    let dest_is_dir = brush_vfs::ambient::metadata(&dest).is_ok_and(|m| m.is_dir());
                    let source_is_dir = brush_vfs::ambient::metadata(source).is_ok_and(|m| m.is_dir());
...
fn copy_extended_attrs(source: &Path, dest: &Path, skip_selinux: bool) -> CopyResult<()> {
    let metadata = brush_vfs::ambient::symlink_metadata(dest)?;
    ...
    if was_readonly {
        ...
        brush_vfs::ambient::set_permissions(dest, perms)?;
    }
    ...
    if was_readonly {
        let mut revert_perms = brush_vfs::ambient::symlink_metadata(dest)?.permissions();
        revert_perms.set_readonly(true);
        brush_vfs::ambient::set_permissions(dest, revert_perms)?;
    }
...
pub(crate) fn copy_attributes(
    ...
    let source_metadata =
        brush_vfs::ambient::symlink_metadata(source).map_err(|e| CpError::IoErrContext(e, context.to_owned()))?;
    ...
    handle_preserve(attributes.ownership, || -> CopyResult<()> {
        ...
        let meta = &brush_vfs::ambient::symlink_metadata(&(dest))
    ...
    handle_preserve(mode, || -> CopyResult<()> {
        ...
        if !brush_vfs::ambient::is_symlink(dest) {
            ...
            brush_vfs::ambient::set_permissions(dest, source_perms)
                .map_err(|e| CpError::IoErrContext(e, context.to_owned()))?;
...
fn backup_dest(dest: &Path, backup_path: &Path, is_dest_symlink: bool) -> CopyResult<()> {
    if is_dest_symlink {
        brush_vfs::ambient::rename(dest, backup_path)?;
    } else {
        match brush_vfs::ambient::remove_file(backup_path) {
            Ok(()) => {}
            Err(err) if err.kind() == io::ErrorKind::NotFound => {}
            Err(err) => return Err(err.into()),
        }
        brush_vfs::ambient::copy(dest, backup_path)?;
    }
    Ok(())
}
        ...
        OverwriteMode::Clobber(cl) | OverwriteMode::Interactive(cl) => {
            match cl {
                ClobberMode::Force => {
                    ...
                    is_symlink_loop(dest) || brush_vfs::ambient::metadata(dest)?.permissions().readonly()
                }
...
fn delete_path(path: &Path, options: &Options) -> CopyResult<()> {
    ...
    #[cfg(windows)]
    if options.force()
        && let Ok(mut perms) = brush_vfs::ambient::metadata(path).map(|m| m.permissions())
    {
        #[allow(clippy::permissions_set_readonly_false)]
        perms.set_readonly(false);
        let _ = brush_vfs::ambient::set_permissions(path, perms);
    }

    match brush_vfs::ambient::remove_file(path) {
        Ok(()) => {
            if options.verbose {
                println!(
                    "{}",
    ...
    match options.copy_mode {
        CopyMode::Link => {
            ...
            if options.dereference(source_in_command_line) && brush_vfs::ambient::is_symlink(source) {
                ...
                brush_vfs::ambient::hard_link(source, dest)
            }
            .map_err(|e| {
                CpError::IoErrContext(
                    e,
        ...
        CopyMode::SymLink => {
            if brush_vfs::ambient::exists(&(dest)) && options.overwrite == OverwriteMode::Clobber(ClobberMode::Force) {
                brush_vfs::ambient::remove_file(dest)?;
            }
        ...
        CopyMode::Update => {
            if brush_vfs::ambient::exists(&(dest)) {
                match options.update {
                    ...
                    UpdateMode::IfOlder => {
                        let dest_metadata = brush_vfs::ambient::symlink_metadata(dest)?;
...
fn copy_file(
    ...
    let dest_target_exists = brush_vfs::ambient::try_exists(&(dest)).unwrap_or(false);
    ...
    if options.preserve_hard_links() && options.copy_mode != CopyMode::Link {
        ...
        if let Some(new_source) = copied_files.get(
            ...
            brush_vfs::ambient::hard_link(new_source, dest)?;
    ...
    let source_metadata = {
        let result = if options.dereference(source_in_command_line) {
            brush_vfs::ambient::metadata(source)
        } else {
            brush_vfs::ambient::symlink_metadata(source)
        };
    ...
    let dest_metadata = brush_vfs::ambient::symlink_metadata(&(dest)).ok();
    ...
    if !dest_is_symlink {
        ...
        brush_vfs::ambient::set_permissions(dest, dest_permissions).ok();
    }
    ...
    let copy_attributes_result = if options.dereference(source_in_command_line) {
        ...
        let src_for_attrs = canonicalize(source, MissingHandling::Normal, ResolveMode::Physical)
            .ok()
            .filter(|p| brush_vfs::ambient::exists(&(p)))
    ...
    } else if source_is_stream && !brush_vfs::ambient::exists(&(source)) {
...
fn copy_helper(
    ...
    if options.parents {
        ...
        if created_parent_dirs.insert(parent.to_path_buf()) {
            brush_vfs::ambient::create_dir_all(parent)?;
        }
    }
...
fn disk_usage(paths: &[PathBuf], recursive: bool) -> io::Result<u64> {
    let mut total = 0;
    for p in paths {
        let md = brush_vfs::ambient::metadata(p)?;
...
fn disk_usage_directory(p: &Path) -> io::Result<u64> {
    ...
    for entry in brush_vfs::ambient::read_dir(p)? {
        let entry = entry?;
