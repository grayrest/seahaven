    ...
    if !raw_pfn.is_null() {
        ...
        unsafe {
                ...
                && source != dest
            {
                ...
                let dest_is_symlink =
                    brush_vfs::ambient::symlink_metadata(dest).is_ok_and(|md| md.file_type().is_symlink());
                if !dest_is_symlink
                    && brush_vfs::ambient::metadata(dest).is_ok_and(|md| !md.permissions().readonly())
                {
                    ...
                    let _ = brush_vfs::ambient::remove_file(dest);
                    error = pfn(src.as_ptr(), dst.as_ptr(), 0);
                }
            }
        }
