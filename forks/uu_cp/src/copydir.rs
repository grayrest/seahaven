...
impl Entry {
    fn new<A: AsRef<Path>>(
        ...
        if no_target_dir {
            ...
            {
                if let Err(e) = brush_vfs::ambient::create_dir_all(context.target) {
                    eprintln!(
                        "{}",
...
fn copy_direntry(
    ...
    // exist, ...
    if source_is_dir && (dest_is_symlink || !brush_vfs::ambient::exists(&(entry.local_to_target))) {
        ...
        match direntry_result {
            Ok(direntry) => {
                ...
                let (entry_is_symlink, entry_is_dir_no_follow) =
                    match brush_vfs::ambient::symlink_metadata(&(direntry_path)) {
                        Ok(metadata) => {
...
fn build_dir(
    ...
    #[cfg(unix)]
    {
        ...
        {
            !brush_vfs::ambient::symlink_metadata(from)?.permissions().mode()
        } else {
