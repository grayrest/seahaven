...
fn display_item_name(
    ...
    if is_long_symlink {
        ...
        match brush_vfs::ambient::read_link(&(path.path())) {
            Ok(target_path) => {
                name.push(" -> ");
                ...
                if should_stat_target {
                    ...
                    match brush_vfs::ambient::canonicalize(absolute_target) {
                        Ok(resolved_target) => {
                            ...
                            let target_display = if let Some(style_manager) = style_manager {
                                let md = match target_data.metadata() {
                                    ...
                                    None => {
                                        brush_vfs::ambient::symlink_metadata(&(target_data.p_buf)).ok().map(Cow::Owned)
                                    }
                                };
...
fn create_hyperlink(name: &OsStr, path: &PathData) -> OsString {
    ...
    let absolute_path = brush_vfs::ambient::canonicalize(path.path()).unwrap_or_default();
