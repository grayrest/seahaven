            ...
            .get_or_init(|| {
                ...
                match get_metadata_with_deref_opt(self.path(), self.must_dereference) {
                    Err(err) => {
                        ...
                        if self.must_dereference
                            && errno == 9i32
                            && let Ok(file) = brush_vfs::ambient::read_link(&(self.path()))
                        {
                            return brush_vfs::ambient::symlink_metadata(&(file)).ok();
                        }
...
pub fn list_with_output<O: LsOutput>(
    ...
    for (pos, path_data) in dirs.iter().enumerate() {
        ...
        let read_dir = match brush_vfs::ambient::read_dir(path_data.path()) {
            Err(err) => {
...
fn enter_directory<O: LsOutput>(
    ...
    while let Some(entry) = current.take().or_else(|| stack.pop()) {
        ...
        let mut current_read_dir = if entry.is_first {
            ...
            match brush_vfs::ambient::read_dir(&entry.path) {
                Err(err) => {
                    output.flush()?;
                    show!(LsError::IOErrorContext(
                        entry.path.clone(),
        ...
        if config.recursive {
                ...
                .rev()
            {
                ...
                match brush_vfs::ambient::read_dir(&child_path) {
                    Err(err) => {
                        output.flush()?;
                        show!(LsError::IOErrorContext(
                            child_path.clone(),
...
fn get_security_context<'a>(
    ...
    if config.smack_supported {
        ...
        let target_path = if must_dereference {
            brush_vfs::ambient::canonicalize(path).unwrap_or_else(|_| path.to_path_buf())
        } else {
            path.to_path_buf()
        };
