...
impl<'a> Output<'a> {
    ...
    fn new_fifo(filename: &Path, settings: &'a Settings) -> UResult<Self> {
        ...
        if settings.seek > 0 {
            Dest::Fifo(brush_vfs::ambient::open(filename)?).seek(settings.seek, settings.obs)?;
        }
...
fn stdout_canonicalized() -> OsString {
    match brush_vfs::ambient::canonicalize(&(Path::new("/dev/stdout"))) {
...
fn is_stdout_redirected_to_seekable_file() -> bool {
    ...
    let p = Path::new(&s);
    match brush_vfs::ambient::open(p) {
        Ok(mut f) => {
...
fn is_fifo(filename: &str) -> bool {
    brush_vfs::ambient::metadata(filename).is_ok_and(|m| m.file_type().is_fifo())
}
