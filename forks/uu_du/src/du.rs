...
impl Stat {
    ...
    fn new_from_dirfd(
        ...
        let std_metadata = brush_vfs::ambient::symlink_metadata(full_path)?;
    ...
    let mut my_stat = match initial_stat {
        Ok(mut s) => {
            ...
            if options.time.is_some()
                && parent_fd.is_some()
                && let Ok(md) = brush_vfs::ambient::symlink_metadata(&s.path)
            {
                s.metadata = md;
            }
            s
...
fn du_regular(
    ...
    if my_stat.metadata.is_dir() {
        let read = match brush_vfs::ambient::read_dir(&my_stat.path) {
            Ok(read) => read,
            Err(e) => {
                print_tx.send(Err(e.map_err_context(
...
fn file_as_vec(filename: impl AsRef<Path>) -> UResult<Vec<String>> {
    let file = brush_vfs::ambient::open(filename)?;
...
fn read_files_from(file_name: &OsStr) -> Result<Vec<PathBuf>, std::io::Error> {
    let reader: Box<dyn BufRead> = if file_name == "-" {
        ...
        match brush_vfs::ambient::open(file_name) {
