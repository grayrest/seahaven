...
fn remove_dir_with_feedback(path: &Path, options: &Options) -> bool {
    match brush_vfs::ambient::remove_dir(path) {
        Ok(_) => {
...
fn handle_parse_error(e: clap::Error, args: &[OsString]) -> Box<dyn UError> {
    let dash_file = args.iter().skip(1).find(|a| {
        os_str_as_bytes(a).is_ok_and(|b| b.len() >= 2 && b[0] == b'-')
            && brush_vfs::ambient::symlink_metadata(a).is_ok()
    });
...
fn count_files(paths: &[&OsStr], recursive: bool) -> u64 {
    ...
    for p in paths {
        let path = Path::new(p);
        if let Ok(md) = brush_vfs::ambient::symlink_metadata(path) {
...
fn count_files_in_directory(p: &Path) -> u64 {
    let entries_count = brush_vfs::ambient::read_dir(p).map_or(0, |entries| {
        entries
            .flatten()
...
pub fn remove(files: &[&OsStr], options: &Options) -> bool {
    ...
    for filename in files {
        ...
        had_err = match brush_vfs::ambient::symlink_metadata(&(file)) {
            Ok(metadata) => {
...
fn is_dir_empty(path: &Path) -> bool {
    brush_vfs::ambient::read_dir(path).is_ok_and(|mut iter| iter.next().is_none())
}
...
fn remove_dir_recursive(
    ...
    #[cfg(any(not(unix), target_os = "redox"))]
    {
        if path.to_str().is_some_and(|s| s.len() > 1000) {
            match brush_vfs::ambient::remove_dir_all(path) {
                Ok(_) => return false,
                Err(e) => {
        ...
        let mut error = false;
        match brush_vfs::ambient::read_dir(path) {
...
fn is_root_path(path: &Path) -> bool {
    ...
    if let Ok(canonical) = brush_vfs::ambient::canonicalize(&(path)) {
...
fn prompt_file(path: &Path, options: &Options) -> bool {
    ...
    let Ok(metadata) = brush_vfs::ambient::symlink_metadata(path) else {
        return true;
    };
