...
pub fn is_readable(path: &Path) -> bool {
    brush_vfs::ambient::metadata(path).is_ok_and(|metadata| is_readable_metadata(&metadata))
}
...
pub fn remove_dir_with_special_cases(path: &Path, options: &Options, error_occurred: bool) -> bool {
    match brush_vfs::ambient::remove_dir(path) {
        ...
        Err(_) if !error_occurred && brush_vfs::ambient::read_dir(&(path)).is_err() => {
...
fn parent_device(path: &Path) -> Option<u64> {
    ...
    brush_vfs::ambient::metadata(parent).ok().map(|m| m.dev())
}
...
pub fn safe_remove_dir_recursive(
    ...
    let (initial_mode, root_dev) = match brush_vfs::ambient::symlink_metadata(path) {
    ...
    let dir_fd = match DirFd::open(path, SymlinkBehavior::Follow) {
        ...
        Err(e) => {
            ...
            if e.kind() == std::io::ErrorKind::PermissionDenied {
                ...
                if brush_vfs::ambient::remove_dir(path).is_ok() {
                    report_verbose_write_error(verbose_removed_directory(path, options));
                    return false;
                }
