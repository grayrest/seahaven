...
pub fn are_files_identical(path1: &Path, path2: &Path) -> io::Result<bool> {
    ...
    let metadata1 = brush_vfs::ambient::metadata(path1)?;
    let metadata2 = brush_vfs::ambient::metadata(path2)?;
    ...
    let file1 = brush_vfs::ambient::open(path1)?;
    let file2 = brush_vfs::ambient::open(path2)?;
...
fn open_file(name: &OsString, line_ending: LineEnding) -> io::Result<LineReader> {
    if name == "-" {
        ...
        if brush_vfs::ambient::metadata(name).is_ok_and(|m| m.is_dir()) {
            return Err(io::Error::other(translate!("comm-error-is-directory")));
        }
        let f = brush_vfs::ambient::open(name)?;
