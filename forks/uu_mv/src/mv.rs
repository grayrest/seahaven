...
fn handle_two_paths(source: &Path, target: &Path, opts: &Options) -> UResult<()> {
    ...
    } else if source_is_dir && brush_vfs::ambient::exists(&(target)) {
...
fn assert_not_same_file(
    ...
    let canonicalized_source = match canonicalize(
        ...
        Ok(source) if brush_vfs::ambient::exists(&(source)) => source,
...
fn move_files_into_dir(files: &[PathBuf], target_dir: &Path, options: &Options) -> UResult<()> {
    ...
    for sourcepath in files {
        if brush_vfs::ambient::symlink_metadata(&(sourcepath)).is_err() {
...
fn rename(
    ...
    if brush_vfs::ambient::exists(&(to)) {
        ...
        if (opts.update == UpdateMode::IfOlder)
            && brush_vfs::ambient::metadata(from)?.modified()? <= brush_vfs::ambient::metadata(to)?.modified()?
        {
            return Ok(());
        }
    ...
    if brush_vfs::ambient::symlink_metadata(&(to))
...
fn rename_with_fallback(
    ...
    brush_vfs::ambient::rename(from, to).or_else(|err| {
        #[cfg(windows)]
        ...
        let metadata = brush_vfs::ambient::symlink_metadata(&(from))?;
...
fn rename_symlink_fallback(from: &Path, to: &Path) -> io::Result<()> {
    let path_symlink_points_to = brush_vfs::ambient::read_link(from)?;
...
fn rename_symlink_fallback(from: &Path, to: &Path) -> io::Result<()> {
    let path_symlink_points_to = brush_vfs::ambient::read_link(from)?;
    if brush_vfs::ambient::exists(&(path_symlink_points_to)) {
...
fn rename_dir_fallback(
    ...
    brush_vfs::ambient::remove_dir_all(from)?;
...
fn create_dir_fail_closed(path: &Path) -> io::Result<()> {
    brush_vfs::ambient::create_dir(path).map_err(|e| {
...
fn copy_dir_contents_recursive(
    ...
    let entries = brush_vfs::ambient::read_dir(from_dir)?;
    ...
    for entry in entries {
        ...
        if brush_vfs::ambient::is_symlink(&from_path) {
            ...
            #[cfg(not(unix))]
            {
                ...
                brush_vfs::ambient::copy(&from_path, &to_path)?;
            }
...
fn copy_file_with_hardlinks_helper(
    ...
    {
        brush_vfs::ambient::hard_link(&existing_target, to)?;
        return Ok(());
    }
...
fn rename_file_fallback(
    ...
    } else if brush_vfs::ambient::exists(&(to)) {
...
fn preserve_ownership(from: &Path, to: &Path) -> io::Result<bool> {
    ...
    let source_meta = brush_vfs::ambient::symlink_metadata(&(from))?;
    let uid = source_meta.uid();
    let gid = source_meta.gid();

    let dest_meta = brush_vfs::ambient::symlink_metadata(&(to))?;
...
fn is_empty_dir(path: &Path) -> bool {
    brush_vfs::ambient::read_dir(path).is_ok_and(|mut contents| contents.next().is_none())
}
