...
fn replace_output_file_in_input_files(
    ...
    if let Some(Ok(output_path)) = output.map(|path| brush_vfs::ambient::canonicalize(&(Path::new(path)))) {
        for file in files {
            if brush_vfs::ambient::canonicalize(&(Path::new(file)))
                .is_ok_and(|file_path| file_path == output_path)
            {
                if let Some(copy) = &copy {
                    *file = copy.clone().into_os_string();
                    ...
                    brush_vfs::ambient::copy(&output_path, &copy_path)
...
impl ClosedTmpFile for ClosedPlainTmpFile {
    ...
    fn reopen(self) -> UResult<Self::Reopened> {
        Ok(PlainTmpMergeInput {
            file: brush_vfs::ambient::open(&self.path).map_err(|error| SortError::OpenTmpFileFailed { error })?,
            path: self.path,
        })
    }
}
...
impl ClosedTmpFile for ClosedCompressedTmpFile {
    ...
    fn reopen(self) -> UResult<Self::Reopened> {
        ...
        let file =
            brush_vfs::ambient::open(&self.path).map_err(|error| SortError::OpenTmpFileFailed { error })?;
