...
impl TmpDirWrapper {
    ...
    pub fn next_file(&mut self) -> UResult<(File, PathBuf)> {
        ...
        Ok((
            brush_vfs::ambient::create(&path).map_err(|error| SortError::OpenTmpFileFailed { error })?,
            path,
        ))
    }
...
fn remove_tmp_dir(path: &Path) -> std::io::Result<()> {
    if let Ok(read_dir) = brush_vfs::ambient::read_dir(path) {
        for file in read_dir.flatten() {
            // if we fail to delete the file here it was probably deleted by another thread
            // in the meantime, but that's ok.
            let _ = brush_vfs::ambient::remove_file(file.path());
        }
    }
    brush_vfs::ambient::remove_dir(path)
}
