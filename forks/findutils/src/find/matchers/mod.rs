...
impl Follow {
    ...
    pub fn metadata_at_depth(
        ...
        Ok(brush_vfs::ambient::symlink_metadata(&(path))?)
    }
}
...
fn get_or_create_file(path: &str) -> Result<File, Box<dyn Error>> {
    let file = brush_vfs::ambient::create(path)?;
    Ok(file)
}
...
fn parse_files0_args(config: &mut Config) -> Result<(), Box<dyn Error>> {
    ...
    if mode == "-" {
        ...
        let mut file =
            brush_vfs::ambient::open(mode).map_err(|e| format!("cannot open '{}' for reading: {}", mode, e))?;
