...
fn write_manifest(src: &Path, dest: &Path, name: &str) -> Result<()> {
    ...
    match manifest.get_mut("dev-dependencies") {
        Some(toml::Value::Table(dev)) => {
...
/// policies on purpose.
const TEST_SESSION_MODULE: &str = r#"// FLATLAND DIVERGENCE, generated by `cargo xtask vendor-fork`.
//
...
fn install_test_session(src: &Path, dest: &Path) -> Result<()> {
    ...
    std::fs::write(
        module_dir.join("flatland_test_session.rs"),
        TEST_SESSION_MODULE,
    )?;
...
fn after_inner_attributes(source: &str) -> usize {
    ...
    for line in source.split_inclusive('\n') {
        ...
        if !continuing && !(trimmed.is_empty() || trimmed.starts_with("//") || trimmed.starts_with("#!")) {
            break;
        }
