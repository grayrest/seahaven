...
| `brush-broker-exec` | The broker-backed `Executor`: spawns a confined child and serves it the session. |
| `brush-shell` | The shell entrypoint, the bundled-command registry and dispatch trampoline, and the confined recipe runner. |
